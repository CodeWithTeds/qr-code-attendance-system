...
<body>
        ...
        <aside class="left-sidebar">
            ...
            <div>
                <div class="brand-logo d-flex align-items-center justify-content-between">
                    <a href="attendance.php" class="text-nowrap logo-img d-flex align-items-center">
                ...
                <nav class="sidebar-nav scroll-sidebar" data-simplebar="">
                    <ul id="sidebarnav">
                        ...
                        <li class="nav-small-cap">
                            <i class="ti ti-dots nav-small-cap-icon fs-4"></i>
                            <span class="hide-menu">FEAUTERES</span>
                        </li>
    ...
    <div class="scanner-con">
        ...
        <!-- Button to toggle camera (without text) -->
        <button id="toggle-camera" class="btn" onclick="toggleCamera()">
            <span id="camera-icon" class="icon-open"></span> <!-- Open camera icon -->
        </button>
...
<script>
    ...
    // Check if camera was previously open
    window.onload = function () {
        const videoElement = document.getElementById('interactive');
        const toggleButton = document.getElementById('toggle-camera');
        
        // Check localStorage to see if camera was open
        if (localStorage.getItem('cameraOpen') === 'true') {
            // If it was open, open the camera again
            openCamera();
            toggleButton.innerHTML = '<span id="camera-icon" class="icon-close"></span>';
        }
    }
    ...
    function toggleCamera() {
        ...
        if (videoElement.style.display === "none") {
            ...
            // Close the camera
            closeCamera();
            toggleButton.innerHTML = '<span id="camera-icon" class="icon-open"></span>';
        }
    }
    ...
    function openCamera() {
        ...
        navigator.mediaDevices.getUserMedia({ video: true })
            .then(function (stream) {
                ...
                // Save the camera state to localStorage
                localStorage.setItem('cameraOpen', 'true');
            })
    ...
    function closeCamera() {
        ...
        // Save the camera state to localStorage
        localStorage.setItem('cameraOpen', 'false');
    }
</script>
    ...
    async initialize() {
        try {
            ...
            this.showMessage('Scanner initialization failed: ' + error.message, 'danger');
        }
    }
    ...
    async startCamera() {
        try {
            ...
            await this.scanner.start(backCamera || cameras[0]);
        } catch (error) {
            throw new Error('Camera access error: ' + error.message);
        }
    }

    async handleScan(qrContent) {
        ...
        try {
            ...
            formData.append('qr_code', qrContent);

            const response = await fetch('./endpoint/add-attendance.php', {
                method: 'POST',
                body: formData
            });
            ...
            } else if (data.includes('already recorded')) {
                this.showMessage('Attendance already recorded for today.', 'warning');
                this.resumeScanning();
            } else if (data.includes('Invalid QR Code')) {
                this.showMessage('Invalid QR Code. Please try again.', 'danger');
                this.resumeScanning();
            } else {
    ...
    showMessage(message, type) {
        ...
        this.messageContainer.querySelector('.alert').className = 'alert alert-dismissible fade show';
        // Add the appropriate alert class
        this.messageContainer.querySelector('.alert').classList.add(`alert-${type}`);
